"""
##############################################################################################################
## ADDED SCRIPTS to Columbia processing scripts BY SHAHRIAR HOOSHMAND
...
if __name__ == "__main__":
    ...
    hd = open_h5_file_read("SH.h5")
    rows = hd.root.metadata.songs.nrows
    result = np.zeros((rows, 109))

    for i in range(rows):
        result[i][0] = get_year(hd,songidx=i)
    ...
    all_functions[1][1](hd)
    result2 = np.zeros((rows, len(all_functions)-2),dtype=object)
    ...
    for i in range(rows):
    #for i in range(2):
        for j in range(1,len(all_functions)-2):
            if i==1:
